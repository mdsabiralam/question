...
import { X, Plus, ArrowUp, ArrowDown, Trash2, Type, Calculator, Image as ImageIcon, BoxSelect, Pencil } from 'lucide-react';
...
export const QuestionEditorModal = ({ question, onSave, onClose }: QuestionEditorModalProps) => {
  ...
  const getDefaultContent = (type: BlockType): any => {
      switch (type) {
          ...
          case 'drawing': return { data: '', canvasData: null };
          default: return '';
      }
  };
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-1/3 border-r bg-gray-50 flex flex-col">
                ...
                <div className="p-3 grid grid-cols-2 gap-2 mt-auto border-t">
                    ...
                    <button onClick={() => handleAddBlock('drawing')} className="flex items-center justify-center gap-1 p-2 bg-white border hover:bg-gray-100 rounded text-xs font-medium"><Pencil className="w-4 h-4" /> Drawing</button>
                </div>
            </div>
...
const getBlockIcon = (type: BlockType) => {
    switch (type) {
        ...
        case 'drawing': return <Pencil className="w-4 h-4 text-orange-500" />;
    }
};

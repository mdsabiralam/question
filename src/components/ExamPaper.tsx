...
import { Trash2, X, Check, Settings2, Eye, Download, Save, Sparkles, Settings, Plus, AlertTriangle, FileEdit, PenTool, FileKey, Wand2 } from 'lucide-react';
...
export const ExamPaper = ({ onOpenGroupSettings, onOpenSetup }: ExamPaperProps) => {
  ...
  // Droppable container for section
  const SectionContainer = ({ section, questions }: { section: Section, questions: Question[] }) => {
      const { setNodeRef } = useDroppable({
          id: section.id,
      });

      return (
          <div ref={setNodeRef} className="space-y-3 min-h-[50px] border border-transparent hover:border-blue-100 rounded transition-colors">
              <SortableContext items={questions.map(q => q.id)} strategy={verticalListSortingStrategy}>
                  {questions.length === 0 ? (
                      ...
                      questions.map((q, index) => {
                          ...
                          return (
                             <SortableExamItem key={q.id} id={q.id}>
                                  <div
        ...
        <div className="border border-dashed border-gray-200 h-full rounded flex-1 flex flex-col">
                ...
                <div className="flex justify-center items-center gap-2 mb-2 relative">
                    ...
                    />
                     <div className="absolute right-0 top-1/2 -translate-y-1/2 flex gap-1">
                        ...
                        {/* Moved Auto-Generate/Add Section Button to Header */}
                        <button
            ...
            {sections.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-gray-400 gap-4">
                    ...
                    <p className="text-sm text-gray-400">Click the wand icon in the header to add a section.</p>
                </div>
            ) : (
                   ...
                   {sections.map((section) => {
                       ...
                       return (
                           <div key={section.id}>
                                ...
                                <SectionContainer section={section} questions={questionsInSection} />
                           </div>
                       );
                   })}
